...
class ShotGroup {
  ...
  public:
    ...
    double robust_bac(void) const
    {
      ...
      // Weight (influence function) is rank^4*(n-rank)
      double numerator = 0;
      double denominator = 0;
      for (unsigned i = 0; i < n; i++) {
        // double influence_function = pow(i + 1, 4) * (n - i - 1);
        double x = (double)i / (n - 1); // x from 0 to 1
        double influence_function = (x - 1) * (x - 1) * x * x / ((x - 1) * (x - 1) + 0.01);
        numerator += r.at(i) * influence_function;
        denominator += influence_function;
      }
